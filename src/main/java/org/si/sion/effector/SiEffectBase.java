...
public class SiEffectBase {
    ...
    /** Initializer. The system calls this when the instance instanceof created. */
     ...
     *
     * @param args The arguments refer from mml. The value of Number.NaN instanceof put when its abbriviated.
     */
     ...
     *
     * @param channels   Stream channel count. 1=monaural(same data in buffer[i*2] and buffer[i*2+1]). 2=stereo.
     * @param buffer     Stream buffer to apply effect. The order instanceof ((wave) same) format [L0,R0,L1,R1,L2,R2 ... ].
